...
#include <limits.h>
#include <string.h>
struct UniqueSrchNode//Memory correct words search query (mcsq)
{
...
void resize_srch_unique_tbl(Thread* thr)
{
	//ht->n_buckets = size<<IND_SHIFTING;

	UniqueTbl* ht = thr->unique_tbl;
...
MemCorSrchQuery*  mem_add_cor_mcsq(Thread* thr, char** pageAr)//(char* sval, CHLEN_T chlen, char** pageAr, wordAr, lenAr, wlen)
{
	...
	int i = str_hash(ht->n_buckets, q, chlen);//_ strSize
	...
	thr->unique_tbl->n_items++;
	if(thr->unique_tbl->n_items > ht->n_buckets)
		resize_srch_unique_tbl(thr);
	return adding_item;
}

void mem_delete_cor_mcsq(Thread* thr, MemCorSrchQuery* sq)//_ should we duduce the size here if items are removed.
{
	CHLIMIT_T chlen = sq->usr_bytes_n;
	char* q = sq->usr_q;
	int i = str_hash(ht->n_buckets, q, chlen);
	MemCorSrchQuery  ***hArs = ht->hArs;
...
{
	int i = str_hash(ht->n_buckets, strKey, chlen);
...
{//
	...
	printf("\ntotal************************************ **************%d", ht->n_buckets);
}


UniqueTbl* create_srch_unique_tbl(int size)
{
	UniqueTbl *ht = malloc(sizeof(UniqueTbl));
	ht->hArs =  malloc(size * sizeof(UniqueSrchNode**));
	ht->hArsLen = size;
	for(int j=0; j<size; j++)
		ht->hArs[j] = malloc((IND_ANDING+1) * sizeof(UniqueSrchNode*));
	
	ht->n_items = 0;
	ht->n_buckets = size<<IND_SHIFTING;
	
	/*ht->intArs =  malloc(sizeof(UniqueSrchNode*));
	ht->intArs[0] = malloc(size*sizeof(UniqueSrchNode));
	ht->intArsLen = 1;
	ht->cur = 0;
